...
|---|---|---|
| Per-(user, channel) session, deterministically keyed | ✅ | `agent:main:{platform}:dm:{chat_id}`, e.g. `agent:main:telegram:dm:100000000`. Stored in `sessions.json`. |
...
| Cron job with channel delivery | ✅ | `hermes cron create --deliver=telegram:100000000`. Same caveat: synthetic `cron_{job_id}_{ts}` session. |
